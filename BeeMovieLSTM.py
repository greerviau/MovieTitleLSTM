...
for i in range(50):
    ...
    m.fit(X, Y, validation_set=0.1, batch_size=128, snapshot_epoch=False, show_metric=True, n_epoch=1, run_id='bee_movie')
    print("-- Testing --")
    print("-- Testing with temp of 1.0 --")
    print(m.generate(50, temperature=1.0, seq_seed=seed))
    print("-- Testing with temp of 0.5 --")
    print(m.generate(50, temperature=0.5, seq_seed=seed))

#final output
seed = random_sequence_from_textfile(path, maxlen)
print("-- Final Script --")
print(m.generate(300, seq_seed = seed))
